...
def pegarFeriados():
    ...
    conMySQLGLPI = MySQLdb.connect(host='192.168.12.6',user='pedreiraumvx',passwd='Admin@#2022!@#',db='glpi') #Criando a conexão
    sql_holidays = f"""
...
def tempoComercial(data_inicio_str,data_fim_str,feriados):
    ...
    while data_inicio_dt.date() <= data_fim_dt.date():
        ...
        if data_inicio_dt.date() in feriados: # pular um dia se for feriado
            ...
            data_inicio_dt = data_inicio_dt.replace(hour=7,minute=0,second=0)
            continue
        if (data_inicio_dt.weekday() == 4 and data_inicio_dt.hour>=16): # ir para o proximo dia se for mais de 16 horas e sexta
            data_inicio_dt = data_inicio_dt + datetime.timedelta(days=1)
            data_inicio_dt = data_inicio_dt.replace(hour=7,minute=0,second=0)
            continue
        if (data_inicio_dt.weekday() in (0,1,2,3) and data_inicio_dt.hour>=17): # ir para o proximo dia se for mais de 17 horas
            data_inicio_dt = data_inicio_dt + datetime.timedelta(days=1)
            data_inicio_dt = data_inicio_dt.replace(hour=7,minute=0,second=0)
            continue
        if data_inicio_dt.hour<7: # se o chamado for aberto antes das 7
            data_inicio_dt = data_inicio_dt.replace(hour=7,minute=0,second=0)
        if data_inicio_dt.date() == data_fim_dt.date(): # para quando a data de inicio e fim for no mesmo dia, caucula a diferença de horas so no dia
        ...
        if data_inicio_dt.date() == data_fim_dt.date(): # diferença de hora do ultimo dia
            ...
            if (data_fim_dt.weekday() == 4 and data_fim_dt.hour >= 16):
                data_fim_dt = data_fim_dt.replace(hour=16,minute=0,second=0)
            if (data_fim_dt.weekday() in (0,1,2,3) and data_fim_dt.hour >= 17):
                data_fim_dt = data_fim_dt.replace(hour=17,minute=0,second=0)
            diferenca = data_fim_dt - data_inicio_dt
            # tempo_segundos+=(diferenca.hour * 3600)
            # tempo_segundos+=(diferenca.minute * 60)
            tempo_segundos+=diferenca.seconds
        ...
        if data_inicio_dt.weekday() in (0,1,2,3): # se não for sexta soma dez horas
...
tempo_previsto = tempoComercial('07-08-2023 09:40:09','18-09-2023 13:46:48',feriados)
